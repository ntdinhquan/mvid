...
export default function UploadStep({ project, setProject, next }: any) {
  ...
  const handleDownloadLink = async () => {
    ...
    try {
      ...
      // Gọi API tải video (đang test local thì dùng localhost)
      const res = await fetch("https://quan2002-mvid-api.hf.space/download-url", {
        method: "POST",
        body: formData,
      });

      if (!res.ok) throw new Error("Không thể tải video từ link này");
      const data = await res.json();

      setProject({
        ...project,
        file: null, // Xóa file local nếu có
        videoServerPath: data.video_path,
        videoUrl: data.preview_url, // Link này để hiện video preview luôn
      });
    } catch (err: any) {
      alert(err.message);
    } finally {
  ...
  return (
    <div className="flex flex-col gap-8 max-w-3xl mx-auto">
      ...
      {!project.videoUrl && (
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4 opacity-70">
            ...
            <span className="text-xs text-gray-500 font-bold tracking-widest uppercase">Hoặc dán link (Chức năng demo thử với link tiktok Việt)</span>
            <div className="h-px bg-gray-300 flex-1"></div>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <button 
              ...
              disabled={isDownloading || !link}
              className="px-6 bg-black text-white rounded-lg hover:bg-gray-800 disabled:opacity-50 font-bold transition shadow-md whitespace-nowrap"
            >
      ...
      {project.videoUrl && (
        <div className="flex flex-col items-center gap-4 bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-sm animate-in fade-in duration-300">
          ...
          <div className="flex justify-between items-center w-full max-w-sm bg-white px-4 py-3 rounded-lg border shadow-sm mt-2">
            <p className="text-sm font-medium text-gray-700 truncate max-w-[200px]" title={project.file ? project.file.name : "Video tải từ link"}>
              🎬 {project.file ? project.file.name : "Video từ Link 🌐"}
            </p>
      ...
      <div className="flex justify-end pt-4 border-t border-gray-100">
        <button
          onClick={next}
          disabled={(!project.file && !project.videoServerPath) || !project.geminiKey.trim()} // <-- ĐÃ SỬA
